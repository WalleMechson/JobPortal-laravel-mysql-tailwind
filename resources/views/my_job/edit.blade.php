...
<x-layout>
    ...
    <x-card>
        <form method="POST" action="{{route('my-jobs.update', ["my_job" => $job])}}"
            ...
            <div class="sm:col-span-2">
                ...
                <x-text-input type="text" name="salary" value="{{$job->salary}}" />
            </div>
            <div class="sm:col-span-2">
                ...
                @foreach ($experienceLevels as $value => $label)
                    <label for="{{ $value }}1" class="flex items-center mb-1">
                        <input type="radio" name="experience" id="{{ $value }}1" value="{{ $value }}"
                            @checked(Str::lower($value) === Str::lower($job->experience)) />
                ...
                @foreach ($categories as $value => $label)
                    <label for="{{ $value }}1" class="flex items-center mb-1">
                        <input type="radio" name="category" id="{{ $value }}1" value="{{ $value }}"
                            @checked(Str::lower($value) === Str::lower($job->category)) />
                        <span class="ml-2">{{ $label }}</span>
                    </label>
                @endforeach
            </div>
